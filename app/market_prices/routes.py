...
def trigger_scrape(_: Farmer = Depends(require_admin), db: Session = Depends(get_db)):
...
def create_price(data: MarketPriceCreate, _: Farmer = Depends(require_admin), db: Session = Depends(get_db)):
